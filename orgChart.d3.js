...
export class OrgChart {
  constructor() {
    ...
    const attrs = {
      ...
      buttonContent: ({ node, state }) => {
        ...
        return `<div style="border:1px solid #E4E2E9;border-radius:3px;padding:3px;font-size:9px;margin:auto auto;background-color:#f6f6f6"> ${icons[
          state.layout
        ](node.children)}  </div>`;
      },
      ...
      linkUpdate: function (d, i, arr) {
        d3.select(this)
          .attr('stroke', function (d) {
            if (d.parent.data.id == 'invisible') return 'none';
            else {
              if (attrs.theme == 'dark')
  ...
  render() {
    ...
    d3.selectAll('.btn-action-button').style('background-color', function () {
      if (attrs.theme == 'dark')
        return attrs.colorsObject.darkZoomButtonBackground;
      else return attrs.colorsObject.lightZoomButtonBackground;
    });
    return this;
  }
  ...
  update({ x0, y0, x = 0, y = 0, width, height }) {
    ...
    const fo = nodeUpdate
      ...
      .classed('root-node', function (d) {
        return d.parent?.id == 'invisible' && d._children?.length > 0;
      });
    ...
    nodeUpdate
      ...
      .attr('opacity', function (d) {
        if (d.data.id == 'invisible') return 0;
        else return 1;
      });
    ...
    nodeUpdate
      ...
      .attr('fill', function (d) {
        if (attrs.theme == 'dark') return attrs.colorsObject.darkNodeBackground;
        else return attrs.colorsObject.lightNodeBackground;
      })
  ...
  restyleForeignObjectElements() {
    ...
    if (attrs.theme == 'dark') tip.attr('class', 'd3-tip-dark');
    else tip.attr('class', 'd3-tip-light');
    //   if (attrs.theme == 'dark') tip.style('background-color', 'red');

    d3.select('.svg-chart-container').call(tip);

    d3.selectAll('.node')
      .on('mouseover', function (e, d) {
        if (d.data.attributes.tooltip == undefined) return;

        if (d.data.attributes.tooltip != '') {
          tip.html(
            (EVENT, d) => `
<div style="max-width:200px">
     ${d.data.attributes.tooltip} </div>
`
          );
